...
class Demo():
    def __init__(self, args):
        ...
        ## NN models
        self.netDepth = EDDeconv(cin=3, cout=1, nf=64, zdim=256, activation=None)
        self.netAlbedo = EDDeconv(cin=3, cout=3, nf=64, zdim=256)
        self.netLight = Encoder(cin=3, cout=4, nf=32)
        self.netView = Encoder(cin=3, cout=6, nf=32)

        self.netDepth = self.netDepth.to(self.device)
        self.netAlbedo = self.netAlbedo.to(self.device)
        self.netLight = self.netLight.to(self.device)
        self.netView = self.netView.to(self.device)
    ...
    def load_checkpoint(self):
        ...
        self.netDepth.load_state_dict(cp['netDepth'])
        self.netAlbedo.load_state_dict(cp['netAlbedo'])
        self.netLight.load_state_dict(cp['netLight'])
        self.netView.load_state_dict(cp['netView'])
    ...
    def run(self, pil_im):
        ...
        with torch.no_grad():
            ...
            ## predict lighting
            canon_light = self.netLight(self.input_im)  # Bx4
            ...
            self.view = self.netView(self.input_im)
            self.view = torch.cat([
